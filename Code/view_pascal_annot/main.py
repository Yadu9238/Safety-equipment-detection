...
ann_dir = os.path.join(args.root_dir, 'final')
#set_dir = os.path.join(args.root_dir, 'ImageSets', 'Main')


def get_image_list(dir):
    image_list = list(os.listdir(dir))
    return image_list
...
def main(args):
    ...
    test = Data(args.root_dir,image_list[0])
    ...
    while True:
        ...
        if args.save_images:
            cv2.imwrite(os.path.join(args.save_dir, image_list[index] + ".png"), image)
